...
from utils import io, image_tools, czi_metadata
...
class OibImageReader(BaseImageReader):
    ...
    def _process_image_and_metadata(self):
        ...
        image_data = image_data.transpose(2, 1, 0, 3, 4)
    ...
    def _extract_metadata_from_associated_file(self):
        ...
        if image_files:
            with TiffFile(image_files[0]) as tif:
                ...
                # # Recalculate X/Y calibration if needed
                # if "XCalibration" in extracted_metadata and extracted_metadata["XCalibration"] == 0:
                #     print("HERE")
                #     extracted_metadata["XCalibration"] = 1. / (float(extracted_metadata["XResolution"]) / 10000)
                # if "YCalibration" in extracted_metadata and extracted_metadata["YCalibration"] == 0:
                #     print("THERE")
                #     extracted_metadata["YCalibration"] = 1. / (float(extracted_metadata["YResolution"]) / 10000)
...
class CziImageReader(BaseImageReader):
    ...
    def _process_image_and_metadata(self):
        ...
        for p, phase_tensor in enumerate(phase_tensors):
            ...
            if num_phases == 2:
                image_name = f'{self._output_folder}\\{self._image_name} {mode[p]}.tif'
            else:
                image_name = f'{self._output_folder}\\{self._image_name} Phase {p + 1}.tif'
...
class TifImageReader(BaseImageReader):
    ...
    def crop(self, start_z=None, end_z=None, start_time=None, end_time=None, 
           start_x=None, end_x=None, start_y=None, end_y=None, start_channel=None, end_channel=None):
        ...
        if start_x is None:
            start_x = 0
        if end_time is None:
            end_time = self._time_count
        if end_z is None:
            end_z = self._z_size
        if end_channel is None:
            end_channel = self._channel_count
        if end_y is None:
            end_y = self._image_data.shape[3]
        if end_x is None:
            end_x = self._image_data.shape[4]

        if start_time >= end_time or start_z >= end_z or start_channel >= end_channel or start_y >= end_y or start_x >= end_x:
            raise ValueError("Invalid cropping parameters. Start values must be less than end values.")
        
        self._image_data = image_tensor[start_time:end_time, start_z:end_z, start_channel:end_channel, start_y:end_y, start_x:end_x]

        return self._image_data
    
    def stack_z_slices(self, stack_channels=True, stack_time=True):
        """
        Consolidates channels and stacks Z-slices of a 5D NumPy array.

        Args:
            data: A 5D NumPy array with shape (time, Z, channels, X, Y).

        Returns:
            A new 5D NumPy array with shape (time, Z * channels, 1, X, Y).
        """

        image_tensor = self.image_data
        min_val = np.min(image_tensor)
        max_val = np.max(image_tensor)
        #image_tensor = ((image_tensor - min_val) / (max_val - min_val) * 255).astype(np.uint8)

        if len(image_tensor.shape) != 5:
            raise ValueError("Input data must be a 5D NumPy array.")

        # Consolidate channels
        if stack_channels:
            channel_stack = []
            for channel in range(image_tensor.shape[2]):
                channel_stack.append(image_tensor[:, :, channel, :, :])
            image_tensor = np.concatenate(channel_stack, axis=1)
            image_tensor = image_tensor[:, :, np.newaxis, :, :]  # Add singleton dimension for channels

        # Consolidate time
        if stack_time:
            time_stack = []
            for time in range(image_tensor.shape[0]):
                time_stack.append(image_tensor[time, :, :, :, :])

            image_tensor = np.concatenate(time_stack, axis=0)

            image_tensor = image_tensor[np.newaxis, :, :, :, :]  # Add singleton dimension for time

        print(image_tensor.shape)
        #tifffile.imsave('D:\\Programming\\Test Data\\Olympus PLA\\TIF\\combined_tensor12.tif', image_tensor, imagej=True, metadata={'axes': 'TZCYX', 'Ranges': (84, 131, 84, 170), 'min': 84, 'max': 131, 'unit': 'um', 'channels': 2, 'frames': 3, 'slices': 6, 'hyperstack': 'true'})

        self._image_data = image_tensor
    
    def denoise(self):
        denoised = self._image_data[0, :, :, :, :]
        denoised = denoise_nl_means(denoised)
        denoised = denoised.astype(np.uint16)
        print(denoised.shape, denoised.dtype)
        denoised = np.expand_dims(denoised, axis=0)
        print(denoised.shape, denoised.dtype)
        print(self._metadata.imagej_compatible_metadata)
    
    def _save_to_tif(self):
        #print(self._image_data.shape, self._output_folder, self._metadata.imagej_compatible_metadata, self._image_name)
        output_path = f"{self._output_folder}{os.path.sep}{self._image_name}_tiffed.tif"
        self.metadata["Info"] = self.info_string(self.image_data)
        image_tools.save_tif(self._image_data, output_path, self._metadata.imagej_compatible_metadata)
