...
from image_tensors import TifImageReader, ImageReader
from utils.io import get_resource_path
from utils.performance_tools import timeit
from params import ModelPaths, DetectionParams
import optimized_tools as opt
import pickle
from numba import jit, prange
from data_structures import Signal, PodosomeCellResult, Podosome
...
class CellPoseDetector:
    def __init__(self, image_array: np.array, model: str = "macrophage", detection_channel: int = 1, 
                 auxilliary_channel: int = 0, diameter: int = 10, do_3d: bool = False, 
        ...
        Args:
            ...
            auxilliary_channel (int): The auxiliary channel for detection. Default is 0.
    ...
    def detect(self, mask_dict=False, **detection_params):
        ...
        Args:
            **param_overrides (dict): Keyword arguments to override the default parameters.
                ...
                - auxilliary_channel (int): The auxiliary channel for detection.
        ...
        if mask_dict:
            pass
            #print("Saving masks as dict...")
        else:
            pass
            #print("Saving masks as numpy array...")

        masks, flows, styles = model.eval(
            image,
            channels=[params.detection_channel, params.auxilliary_channel],
        ...
        # Trim the masks and other outputs back to the original size
...
class MacrophageDetector:
    ...
    def get_masks(self):
        ...
        # Display the image with random-colored transparent masks
        # plt.imshow(image_with_masks_rgba)
        # plt.title("Image with Random Colored Transparent Masks")
        # plt.axis('off')  # Hide axes
        # plt.show()
...
class PodosomeDetector:
    ...
    def detect(self):
        ...
        masks_3d = CellPoseDetector(image_3d_normalized, do_3d=True, diameter=10, 
                                      ...
                                      gpu=True, flow_threshold=0.4, cellprob_threshold=0.0).detect()
...
if __name__ == "__main__":
    ...
    image_data = ImageReader(r"D:\Microscopy Testing\20241014 Single Cut Out PLA\Clathrin vs Drebrin PLA Series 8_Cell_2.tif").image_data()

    for image in image_data:
        masks = PodosomeDetector(image).detect()

        print(masks.shape)

        cell_podosomes = PodosomeManager(masks).process()

        print(len(cell_podosomes.podosomes))
