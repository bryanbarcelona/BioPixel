...
from macropy.utils import image_tools

def get_square_crop_params(masks: np.ndarray, mask_id: int) -> tuple | np.ndarray:
    """
    ...
    if len(rows) > 0:
        ...
        x_distance = max_col - min_col
        y_distance = max_row - min_row
        
        # Calculate the size of the square crop with padding
        square_crop_size = int(max(x_distance, y_distance) * 1.14)

        # Calculate the center of the bounding box
        center_x = (min_col + max_col) / 2
        center_y = (min_row + max_row) / 2

        # Calculate the half-size of the cropped area
        half_crop_size = square_crop_size // 2

        # Calculate the initial crop box coordinates
        crop_left = int(center_x - half_crop_size)
        crop_right = int(center_x + half_crop_size)
        crop_top = int(center_y - half_crop_size)
        crop_bottom = int(center_y + half_crop_size)

        # Adjust crop box coordinates to stay within image boundaries
        crop_left = max(crop_left, 0)
        crop_right = min(crop_right, masks.shape[1])
        crop_top = max(crop_top, 0)
        crop_bottom = min(crop_bottom, masks.shape[0])

        # Calculate the actual cropped image size
        cropped_width = crop_right - crop_left
        cropped_height = crop_bottom - crop_top

        # If the cropped dimensions are not square, adjust the crop box to make it square
        if cropped_width != cropped_height:
            diff = abs(cropped_width - cropped_height)
            if cropped_width > cropped_height:
                if crop_bottom + diff <= masks.shape[0]:
                    crop_bottom = crop_bottom + diff
                else:
                    crop_top = max(crop_top - diff, 0)
            else:
                if crop_right + diff <= masks.shape[1]:
                    crop_right = crop_right + diff
                else:
                    crop_left = max(crop_left - diff, 0)

    else:
        print(f"Value {mask_id} not found in the array.")

    #print(f"Center box is: {crop_top}, {crop_bottom}, {crop_left}, {crop_right}")
    return (crop_top, crop_bottom, crop_left, crop_right), zero_mask
